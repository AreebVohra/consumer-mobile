...
import {
  BLOCKING_CODES, NFC_ENROLLMENT_ERRORS, NFC_ENROLLMENT_RISK_ERRORS, SCORE_DATA_VERIFIED, BACK_COUNTRIES, MAF_ASSETS,
...
const GiftCard = ({ navigation }) => {
  ...
  const {
    ...
    currentUserScore, isConsumerMerchantSpendLimitResolved, isMAFQualifiedLoading,
  ...
  const {
    ...
    lean, mobileLimit, merchantSpendLimitReason, requiresNfcMAF,
  } = currentUserScore || {};
  ...
  const bgImage = { uri: `${MAF_ASSETS.HERO}-${currLang}.png` };
  ...
  useEffect(() => {
    if ((!currentUser.hasIdentitiesUploaded || currentUser.isIdExpired) && isConsumerMerchantSpendLimitResolved && consumerMerchantSpendLimit >= consumerMinimumSpendLimit) {
    ...
    nfcSupported, consumerMerchantSpendLimit, isConsumerMerchantSpendLimitResolved, startNFCFlow,
  ]);
              ...
              <View style={[styles.columnFlex, styles.marginTop1]}>
                ...
                {[1, 2, 3, 4, 5].map(i => (
                  <View style={[styles.howItWorksRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                    ...
                    <View style={[styles.marginLeft2, styles.marginTop1, styles.maxWidth90, styles.paddingHorizontal5]}><Text style={{ textAlign: isRTL ? 'right' : 'left' }}>{t(`common.howGCWorks${i}`)}</Text></View>
                  </View>
                ))}
    ...
    <View style={styles.margin3}>
      ...
      <View style={[styles.flex, styles.justifySpaceBtw, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
        <View style={[styles.rowFlex, styles.alignItemsCenter]}>
          ...
          <Text
            onPress={setShowHIW}
            style={[{ marginBottom: 1, marginLeft: 5, fontSize: 12, fontWeight: '400', lineHeight: 15, color: '#411361', textDecorationLine: showHIW ? 'underline' : 'none' }]}
          >
            {t('common.howGcWorks')}
          </Text>
        </View>
        <View style={[styles.rowFlex, styles.alignItemsCenter]}>
          ...
          <Text
            ...
            style={[{ marginBottom: 1, marginLeft: 5, fontSize: 12, fontWeight: '400', lineHeight: 15, color: '#411361' }]}
          >
  ...
  if ((!currentUser.hasIdentitiesUploaded || currentUser.isIdExpired) && isConsumerMerchantSpendLimitResolved && (startNFCFlow || (consumerMerchantSpendLimit && consumerMerchantSpendLimit >= consumerMinimumSpendLimit))) {
  ...
  const qualifyList = [
    ...
    },
    {
      ...
      || [BLOCKING_CODES.MOB_OOM, BLOCKING_CODES.DEF].includes(merchantSpendLimitReason))
    ...
    <Layout style={styles.layout} level="1">
      <ScrollView overScrollMode="never">
        ...
        {!isMAFQualifiedLoading ? (
          <>
            {uaeCustomer && (
              !(isConsumerMerchantSpendLimitResolved && consumerMerchantSpendLimit < consumerMinimumSpendLimit) && (
                <View style={[styles.gcText]}>
                  <Text style={[styles.gcTitle, styles.centerAlign]}>{t('common.buyGCOnSpotii')}</Text>
                  <Text style={[styles.gcDescription, styles.centerAlign, styles.marginTop3]}>{t('common.pay25OfGC')}</Text>
                </View>
              )
            )}
            <View style={styles.margin3}>
              {(isConsumerMerchantSpendLimitResolved && consumerMerchantSpendLimit < consumerMinimumSpendLimit)
                ? <HowToQualify qualifyList={qualifyList} navigation={navigation} /> : (
                  <>
                    {gcGiftCard()}
                    {gcSlider()}
                    <Text style={[styles.margin2, styles.mediumSubText, { textAlign: isRTL ? 'right' : 'left' }]}>
